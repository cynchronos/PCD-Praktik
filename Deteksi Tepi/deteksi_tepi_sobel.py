...
kernelX = np.array([[0,1,0],[1,-4,1],[0,1,0]]) #laplace 1
kernelY = np.array([[1,1,1],[1,-8,1],[1,1,1]]) #laplace 2
...
for brs in range(1,jml_baris-1):
    for klm in range(1,jml_kolom-1):
        ...
        for i in range(2):
            ...
            temp_konvolusi = np.abs(np.round((a + b + c + d + e + f + g + h + i) / jum_kernel))
            ...
            # Tambahkan Konvolusi x dan y
            konvolusi += temp_konvolusi
